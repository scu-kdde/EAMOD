...
for data_num =3:13
    ...
    for first_class=length(classes_all)
        time_all =[];
        %for t = 25
        tic
        ...
        for run=1:tot_run  %%% Total number of time to rerun the experiment
            ...
            for f=1:tot_fold
                ...
                eta_all={};
                num_kern = 2;
                c_array = 10.^(-1:0.05:1); %.^(-1:0.05:1); %%% Regularization Parameter
                ...
                for sigm = 1:length(sig_array)
                    %for k1=2:3
                        ...
                        for c=1:length(c_array)
                            ...
                            final = {['g' num2str(sig_array(sigm))],'l'};
                            count=count+1;
                            [data_num first_class count]
                            if c==1
                                [model,model2,eta]=create_model(train_data,train_lbls,final,c_array(c));
                                alp_all = [alp_all,model2.alpha];
                                eta_value{count} = eta;
                            else
                                [model,model2,aa,bb,eta]=create_model2(train_data,train_lbls,final,c_array(c),c_array(c-1),alp_all(:,c-1));
                                alp_all = [alp_all,model2.alpha];
                ...
                [~,max_ind2]=find(gm==max_val);
                max_ind2all{f,:} = max_ind2;
                res.eta_all = eta_all;
            ...
            [max_val opt_ind1] = max(gmean);
            res.eta = max_eta;
            res.max_ind = max_ind2all;
            %%% For Training
